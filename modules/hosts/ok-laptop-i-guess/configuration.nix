{ self, inputs, ... }: {
  ...
  flake.nixosModules.host-ok-laptop-i-guess-configuration = { pkgs, lib, hostConfig, ... }: {
    
    imports = [
      self.nixosModules.host-ok-laptop-i-guess-hardware
    ];
    ...
    users.users.${hostConfig.user.name}.packages = with pkgs; [
      kdePackages.kate
    ];
